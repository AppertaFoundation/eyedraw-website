...
ED.Views.DoodlePopup = (function() {
	 ...
	 * @type {Number}
	 */
	var animateTime = 1; // MSC reduced time
	...
	DoodlePopup.prototype.update = function(show) {
		if (show && this.drawing.selectedDoodle.noPopup) {}
		else if (show && this.drawing.selectedDoodle) {
			this.render();
			this.show();
		} else {
			this.hide();
...
ED.Views.SelectedDoodle = (function() {
	...
	SelectedDoodle.prototype.render = function() {
        ...
        for (var i=doodles.length-1; i>=0; i--) {
		    if (doodles[i].className == 'MemberConnector') doodles.splice(i,1);
		}
