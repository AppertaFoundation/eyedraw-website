...
{
    // Slider2D Properties (Establish before superclass call so values are available for setPropertyDefaults)
//    this.halfHeight =  6;
//    this.halfLength = 200;
...
ED.Slider2D.prototype.setPropertyDefaults = function()
{
    ...
    this.gridSpacing = 10;
    this.rangeOfOriginX = new ED.Range(-400, +400);
    this.rangeOfOriginY = new ED.Range(-400, +400);
    this.isShowHighlight = false;
    this.animationArray = {sphere: "10"};
}
 ...
 * @returns {String} value of parameter
 */
ED.Slider2D.prototype.getParameter = function(_parameter)
{
    var returnValue;
    ...
    switch (_parameter)
    {
        // Sphere
        case 'sphere':
            // Gives -20 to +20 range in 0.25 increments
            returnValue = (-Math.floor(this.originY/5) * 25/100).toFixed(2);
            returnValue = (this.originY < 0?'+':'') + returnValue;
            break;
        // Cylinder
        case 'cylinder':
            // Gives -20 to +20 range in 0.25 increments
            returnValue = (Math.floor(this.originX/5) * 25/100).toFixed(2);
            returnValue = (this.originX > 0?'+':'') + returnValue;
            break;
        default:
            returnValue = "";
            break;
 ...
 * @param {String} _value New value of parameter
 */
ED.Slider2D.prototype.setParameter = function(_parameter, _value)
{
    switch (_parameter)
    {
        // Sphere
        case 'sphere':
            this.originY = -parseFloat(_value) * 20;
            break;
        // Cylinder
        case 'cylinder':
            this.originX = parseFloat(_value) * 20;
            break;
        default:
            break
    }
...
{
    // GraphAxes Properties (Establish before superclass call so values are available for setPropertyDefaults)
    //    this.halfHeight =  6;
    //    this.halfLength = 200;
 ...
 * Sets default parameters
 */
ED.GraphAxes.prototype.setParameterDefaults = function()
{
}
